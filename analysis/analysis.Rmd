...
print(
  sprintf(
    "End of anti-TB treatment: %i observations (%i percent).",
    nrow(qol_end),
    round(100 * nrow(qol_end) / n_distinct(qol$record_id))
  )
)

print(
  sprintf(
    "Post treatment: %i observations (%i percent).",
    nrow(qol_post),
    round(100 * nrow(qol_post) / n_distinct(qol$record_id))
  )
)
```

**Table 1**: Number and proportion of patients per site.
...
**Table 2**: Number and proportion of patients with missing follow-up (MFU) study visits.
...
**Table 3**: Number and proportion of patients who died.
...
**Table 5**: Distribution of patient characteristics by time.
...
**Table 6**: Number of patients with non-fatal serious adverse events (AE). 
...
- Few patients with AE were have missing follow-up visits.

&nbsp;

**Table 7**: Number and proportion of patients who received mental health treatments. 
...
**Table 8**: List of visits of patients receiving mental health treatment, their mental health outcomes (PHQ9-S and SF12-MCS) and whether they died or had missing follow-up visits.
...
**Table 10**: QoL scores at the start of anti-TB treatment.
...
**Table 11**: QoL scores at the end of anti-TB treatment.
...
**Table 12**: QoL scores post anti-TB treatment.
...
**Figure 6**: Trajectories of patients who were depressed (defined as PHQ9-S > 10) once during the study.
...
We impute the missing values in the continuous QoL scores.
...
**Figure 7**: Imputation check for a random sample of 30 patients, showing the imputed majority label across datasets.
...
We invert PHQ9-S and SGRQ-TS using (max_y - y) so that all changes>0 imply an QoL improvement. 
We report the median and 95% credible interval (CrI) of the posterior draws for each model parameter estimate.
...
where beta is the estimated effect, n_fu is the number of patients *with* and n_nfu is the number of patients *without* follow-up or death.

**Figure 9:** Estimated change in QoL (grey) for patients with follow-up visits vs offsetting hypothetical change (red) in QoL for patients without missing follow-up visit or death.
...
- QoL decrease for patients without follow-up would need to be extremely large to offset improvements in patients with follow-up visits so that there is no average effect.
...
**Figure 10**: Estimated change in QoL outcomes for fully-imputed dataset including patients with missing follow-up visits or death.
...
- The latter can be expected because imputation of outcomes in missing follow-up patients is primarily based on the estimated effect in patients with follow-up, thus re-inforcing their estimated effect.
...
**Figure 11**: Probability that patient characteristics are associated with lower or higher QoL scores. 
...
- The plot shows whether patient characteristics are associated with a higher (blue bar left of the middle dashed line) or lower in QoL (red bar right of the middle dashed line).
- Bars left (20%) or right (80%) of the outer dashed lines can be considered significant associations, i.e. the odds for the association are larger than 80%/20% = 4.
- Age<30 is typically associated with higher QoL scores. 
- Female sex is typically associated with lower QoL scores.
- Relapse TB-cases, drinking, and smoking are typically associated with lower QoL scores.
- For HIV+ and MDR, the associations are more ambigious.
...
**Table 13** Estimation results for the change in the QoL z-scores by predictor and time. Median and 95%-CrI and posterior probability that change dz is smaller or greater than zero.
...
**Table 14**: Estimated association of subsequent death with QoL.
...
**Table 15**: Estimated association of subsequent missing follow-up with QoL.
...
**Table 16**: Estimated association of non-fatal adverse events with QoL.
...
**Table 17**: Estimated association of meantal health treatment with QoL.
...
**Figure 12**: Estimated proportion of patients with impaired QoL. Pr = Bayesian probability that the proportions are smaller at the end vs start of and end of vs post treatment.
...
**Figure 13**: Estimated proportion of patients with depression for different cutoff values. Pr = Bayesian probability that the proportions are smaller at the end of and post treatment vs the start and end of treatment, respectively.
...
- But relatively, the results are very similar (similar significant changes).
...
**Figure 14**: Estimated change in QoL z-score (median as dot, 95%-CrI as lines) per quantile at the end of anti-TB treatment by patient characteristic.
...
- Low mental health scores and SGRQ-TS are associated with relapse TB cases.
