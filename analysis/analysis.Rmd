...
qol <- readRDS("../tb_care/data-clean/clean-cohort-data.rds") %>%
  filter(time %in% c("Start", "End", "Post")) %>%
  ...
  mutate(
    ...
    tbd_clin_diag = if_else(site == "Mozambique", NA, tbd_clin_diag),
...
var_names <- c(
  "age" = "Age",
  "sex" = "Sex",
  "tbd_clin_diag" = "Type of TB diagnosis",
...
```{r depressed completing treatment}
qol %>%
  filter(time %in% c("Start", "End")) %>%
  dplyr::select(
    record_id, time, phq9_score_bin,
    missing_fup_yn, pending_visit_yn
  ) %>%
  pivot_wider(
    names_from = time,
    values_from = c(phq9_score_bin, missing_fup_yn, pending_visit_yn)
  ) %>%
  summarise(
    not_depr_start = sum(phq9_score_bin_Start == 0, na.rm = TRUE),
    not_depr_fup = sum(
      !missing_fup_yn_End & phq9_score_bin_Start == 0,
      na.rm = TRUE
    ),
    depr_start = sum(phq9_score_bin_Start, na.rm = TRUE),
    depr_with_fup = sum(
      !missing_fup_yn_End & phq9_score_bin_Start == 1,
      na.rm = TRUE
    )
  ) %>%
  mutate(
    prop_not_depr = round(not_depr_fup / not_depr_start * 100, 0),
    prop_depr = round(depr_with_fup / depr_start * 100, 0)
  )
```
...
assoc_bin_pl <- assoc_bin %>%
  ...
  ggplot(aes(y = prob, x = name, fill = direction)) +
  facet_grid(outcome ~ predictor) +
  geom_bar(stat = "identity", position = position_stack(reverse = TRUE)) +
  geom_hline(aes(yintercept = 20), linetype = "dashed", alpha = .5) +
  geom_hline(aes(yintercept = 50), linetype = "dashed", alpha = .5) +
  geom_hline(aes(yintercept = 80), linetype = "dashed", alpha = .5) +
  scale_y_continuous(expand = c(0, 0), breaks = c(0, 20, 50, 80, 100)) +
...
ggsave(
  plot = assoc_bin_pl,
  filename = "results/association-direction",
  width = 17 / cm(1), height = 25 / cm(1),
  device = "tiff",
  compress = "lzw"
)
```
...
impairment_time_plot <- function(
    samples = qol_bin_smpls,
    data = unnest(qol_imp_bin, data),
    ann_above = FALSE) {
  ...
  pp <- samples %>%
    ...
    mutate(
      value = scales::percent(value / 100, accuracy = 1, suffix = ""),
      value = if_else(
        value == "100",
        ">99 * \"%\"",
        paste0("==", value, " * \"%\"")
  ...
  pp <- rbind(
    pp_ypos_end %>% mutate(time = "End"),
    pp_ypos_post %>% mutate(time = "Post")
  ) %>%
    left_join(pp, by = c("outcome", "time")) %>%
    arrange(outcome, time) %>%
    mutate(
      y_pos = p_pos,
      x_min_pos = c(.75, 1, 1.75, 2, 2.75, 3, 3.75, 4, 4.75, 5, 5.75, 6),
      x_max_pos = c(1, 1.25, 2, 2.25, 3, 3.25, 4, 4.25, 5, 5.25, 6, 6.25),
      x_pos = (x_min_pos + x_max_pos) / 2
    )
  ...
  pl <- prop %>%
    ...
    geom_text(
      data = pp,
      mapping = aes(x = x_pos, y = y_pos + 2.5, label = value, color = time),
      parse = TRUE,
      vjust = -0.5,
      size = 7 / cm(1)
    ) +
    annotate(
      "text",
      x = .5, y = if_else(ann_above, 108, 98),
      label = expression("P"[ES] * " = Probability (End < Start)"),
      size = 7 / cm(1),
      hjust = 0, color = RColorBrewer::brewer.pal(3, "Set2")[2]
    ) +
    annotate(
      "text",
      x = .5, y = if_else(ann_above, 103, 93),
      label = expression("P"[PE] * " = Probability (Post < End)"),
      size = 7 / cm(1),
      hjust = 0, color = RColorBrewer::brewer.pal(3, "Set2")[3]
    ) +
    scale_y_continuous(
      breaks = scales::breaks_pretty(5),
      expand = expansion(mult = c(0, if_else(ann_above, 0, 0.025)))
    ) +
    scale_fill_brewer(palette = "Set2") +
    scale_color_manual(values = RColorBrewer::brewer.pal(3, "Set2")[2:3]) +
    coord_cartesian(ylim = c(0, NA), clip = "off") +
    labs(y = "Proportion (%)") +
    theme_custom(8) +
    theme(
...
qol_imp_bin_ctry <- qol_imp_bin_ctry %>%
  unnest(c(data)) %>%
  rename(country = site)

# plots
for (ctry in levels(qol$site)) {
  pl_comp_ctry <- impairment_time_plot(
    samples = filter(qol_bin_smpls_ctry, country == ctry),
    data = filter(qol_imp_bin_ctry, country == ctry),
    ann_above = if_else(ctry == "Mozambique", TRUE, FALSE)
  ) +
    ggtitle(ctry) +
    theme(
...
phq9_bin_pp <- rbind(
  ...
  left_join(phq9_bin_pp, by = c("outcome", "time")) %>%
  arrange(outcome, time) %>%
  mutate(
    y_pos = p_pos,
    x_min_pos = c(.75, 1, 1.75, 2),
    x_max_pos = c(1, 1.25, 2, 2.25),
    x_pos = (x_min_pos + x_max_pos) / 2
  )
...
pl_phq9_comp <- phq9_imp_prop %>%
  ...
  ggsignif::geom_signif(
    annotations = phq9_bin_pp$time,
    y_position = phq9_bin_pp$y_pos + 3,
    xmin = phq9_bin_pp$x_min_pos,
    xmax = phq9_bin_pp$x_max_pos,
    textsize = 0,
    tip_length = 0.01
  ) +
  geom_text(
...
### Sensitivity analysis: Missing follow-up and patient characteristics

```{r missing follow-up and patient characteristics, echo = FALSE}
# data
mfu_patients <- qol %>%
  filter(time == "End") %>%
  filter(!death_yn, !pending_visit_yn) %>%
  dplyr::select(record_id, missing_fup_yn) %>%
  na.omit()
mfu_characteristics <- qol %>%
  filter(time == "Start") %>%
  mutate(
    alc_bin = if_else(as_alc_calc > 0, "Yes", "No"),
    tobacco_bin = if_else(as_tobacco_calc > 0, "Yes", "No"),
  ...
  dplyr::select(
    record_id, site,
    names(predictor_bin_sub)
  )
mfu_df <- mfu_patients %>%
  left_join(mfu_characteristics, by = "record_id")

# model
mfu_mod <- brm(
  missing_fup_yn ~ (1 | site) + age_bin + sex + hiv + mdr +
    tbd_pat_cat + alc_bin + tobacco_bin,
  data = mfu_df,
  family = bernoulli(),
  seed = 12345
)
...
mfu_est <- mfu_mod %>%
  spread_draws(`b_.*`, regex = TRUE) %>%
  dplyr::select(-b_Intercept, -.chain, -.iteration) %>%
  pivot_longer(
    cols = -`.draw`,
    names_to = "variable",
    values_to = "value"
  ...
  bind_rows(
    tibble(
      variable = c(
        "b_siteSouthAfrica", "b_age_binNo", "b_sexMale",
        "b_hivNegative", "b_mdrNo", "b_tbd_pat_catNew",
        "b_alc_binNo", "b_tobacco_binNo"
      ),
      value = rep(0, 8)
    )
  ) %>%
  mutate(
    variable = recode_factor(variable, !!!c(
      "b_siteSouthAfrica" = "South Africa",
      "b_siteMalawi" = "Malawi",
      "b_siteMozambique" = "Mozambique",
      "b_siteZambia" = "Zambia",
      "b_siteZimbabwe" = "Zimbabwe",
      "b_age_binYes" = "Age < 30",
      "b_age_binNo" = "Age \u2265 30",
      "b_sexFemale" = "Female",
      "b_sexMale" = "Male",
      ...
      "b_mdrYes" = "MDR-TB",
      "b_mdrNo" = "Non-MDR-TB",
      "b_tbd_pat_catRelapse" = "TB history",
      "b_tbd_pat_catNew" = "No TB history",
      "b_alc_binYes" = "Alcohol use",
      "b_alc_binNo" = "No alcohol use",
      "b_tobacco_binYes" = "Tobacco use",
      "b_tobacco_binNo" = "No tobacco use"
    )),
    or = exp(value)
  )
...
mfu_pl <- mfu_est %>%
  ...
  ungroup() %>%
  ggplot(aes(y = variable, x = or, shape = ref_cat)) +
  stat_pointinterval(
  ...
  labs(x = "Odds ratio (median and 95%-CrI)") +
  theme_custom() +
  theme(
...
save_plot(
  mfu_pl,
  pdf_file = "results/missing-follow-up-characteristics.png",
  w = 10, h = 8
)

# check MDR-TB proportion among missing follow-up patients
mfu_df %>%
  filter(mdr == "Yes") %>%
  filter(missing_fup_yn)
```